...
### 通知办理智能体（ClearFlow 办理智能体）
在结构化解析与 RAG 基础上，项目新增 **“通知办理智能体”** 作为完整闭环的执行层：  
它会将通知内容转为“可执行清单”，并提供 **证据链、风险提示、时间线与材料清单**。  
核心输出包含：办理事项、适用对象、材料清单、办理流程、注意事项、风险提示、证据链片段与置信度评分。  
对应接口：`POST /agent/run`，前端已提供“通知办理智能体”页面用于交互展示与可视化。
