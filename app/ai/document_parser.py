...
def parse_document(original_text: str, user_id: int) -> ChatMessageBase:
    ...
    JSON 对象的键必须严格如下：
    {
        ...
        "required_materials": "办理需要准备哪些材料？（请尽量列出清单）",
        "handling_process": "具体的办理流程分为哪几步？",
    ...
    try:
        ...
        return ChatMessageBase(
            ...
            original_text_mapping=str(parsed_data.get("original_text_mapping")) if parsed_data.get("original_text_mapping") else None
        )
...
def rewrite_document(original_text: str, target_audience: str, user_id: int) -> ChatMessageBase:
    ...
    {{
        ...
        "required_materials": "办理需要准备哪些材料？",
        "handling_process": "具体的办理流程？（用符合该群体理解能力的步骤描述）",
        "precautions": "有哪些需要特别注意的事项？",
