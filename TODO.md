...
### 客户端UI
- [ ] 主页-最近解析栏在未滑出时应该完全隐藏.
- [ ] 给主页右侧的中央文件栏提供一个和左边的最近解析抽拉按钮一样样式的抽拉按钮,可以平滑的抽拉.
- [ ] 解决header的搜索框下的标签显示为`[object Object]`的问题
- [ ] 制作一个sidebar两个模式平滑切换的动画(像液体一样),header也跟着平滑切换.
### 人力完成(UI)
- [ ] 莫兰迪色系数据统计的引入.
- [ ] 响应式布局的实现,争取在移动端适配.
...
- [ ] "刷剧"体验,提供一个根据用户个性(职业需求)进行某种算法和推送文章的逻辑,用户可以上下滑动像看小说一样看文章.
